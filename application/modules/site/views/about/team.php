 <div class="row">
    
     <div class="agents text-center">
        <!-- <div class="content-title">
            <h2>Our team</h2>
        </div -->

        <div class="col-md-7 col-lg-8">
            <section class="row">
                <article class="col-sm-4 col-lg-5" style="margin-top:12px;">
                    <div class="thumbnail">
                        <a href="#"><img src="<?php echo base_url();?>assets/themes/realta/img/preview/agents/Portrait Dad.jpg" class="img-responsive agent-img" alt=""></a>

                        <div class="caption text-center">
                            <p>i.haggarty@wsfn.com.au<br/>
                                0414 639 738</p>
                        </div>
                    </div>
                </article>
                <article class="col-sm-8 col-lg-7">
                    <h4 class="text-left">Ian Haggarty</h4>
                    <div style="text-align:justify">
                   <p> One of the longest serving agents in the Hills District, Ian is involved in all facets of real estate including planning, acquisitions, development and sales.</p>
                   <p>Ian Haggarty is a well-known and trusted professional renowned for having an impeccable sense of integrity, he has built a successful business based largely on referrals and repeat customers.</p>
<p>With unrivalled experience, he has been serving generations of families in the area since 1980 and is an integral part of the community not only on a business level, but also through his involvement with several charities and sponsorships including many of the local sporting clubs.</p>
                    <p>Highly respected for his business ethics and kind nature, Ian is admired for providing clients with reliable and well-informed advice.
</p>
                    </div>
                
                </article>
                
            </section>
            <section class="row">
                <article class="col-sm-4 col-lg-5" style="margin-top:12px;">
                    <div class="thumbnail">
                        <a href="#"><img src="<?php echo base_url();?>assets/themes/realta/img/preview/agents/Portrait Scott.jpg" class="img-responsive agent-img" alt=""></a>

                        <div class="caption text-center">
                            <p>s.haggarty@wsfn.com.au<br/>
                                0414 072 084</p>
                        </div>
                    </div>

                </article>
                <article class="col-sm-8 col-lg-7">
                   <h4 class="text-left">Scott Haggarty</h4>
                    <div style="text-align:justify">
                        <p>As a local resident Scott Haggarty has a strong understanding of the Hills District and has developed a phenomenal reputation as a trusted advisor.</p>

                <p>Scott´s local knowledge is of tremendous value to the First National team and of great benefit to his customers. As a local home owner Scott regards the area´s family-friendly communities, open space and outstanding amenities as among the most appealing attributes for potential buyers. His approach brings a uniquely personal perspective to clients throughout the Hills District.</p>

                <p>Scott thrives on dealing with people and genuinely enjoys being involved with helping clients achieve their property dreams. His down-to-earth attitude and friendly persona, matched by a determination to deliver exceptional quality service, instils clients with confidence and removes any trepidation when it comes to entrusting Scott with the sale of their property.</p>

                <p>He enjoys renovating properties and is often found indulging in his passion for sports. He is understanding of clients´ needs and believes the First National groups professional marketing strategies combined with the dedication of its people are key to a result beyond expectations in every instance.</p>
                    </div>
                </article>
            </section>
           
        </div>
        <div class="col-md-5 col-lg-4">
                 <section class="row" >
                     <article class="col-md-12">
                     	<div class="categories">
                       		<h4 style="text-align:justify "><strong>Proud sponsors</strong></h4>
                            <div style="text-align:justify;">
                                <p>The Hills Club</p>
                                <p>Waves Fitness & Aquatic Centre</p>
                                <p>Excelsior Public School</p>
                            </div>
                        </div>
                    </article>
                </section>
                <br>
                <section class="row">
                     <article class="col-md-12">
                     	<!-- <iframe  height="249px" width="98%" style="margin-left:-2%" src="https://www.youtube.com/embed/72pb7klBTDE?loop=1&playlist=72pb7klBTDE" frameborder="0" allowfullscreen></iframe> -->
                       <div class="youtube" id="72pb7klBTDE"></div>
                    </article>
                </section>
                <br>
                 <section class="row">
                     <article class="col-md-12">
                     	<!-- <div class="youtube" id="sjQShSoR7e4" style="width: 100%; height: 249px;"></div>
                       <iframe height="249px" width="98%" style="margin-left:-2%" src="https://www.youtube.com/embed/9gnXba2_CNo?loop=1&playlist=9gnXba2_CNo" frameborder="0" allowfullscreen></iframe> -->
                       <div class="youtube" id="9gnXba2_CNo"></div>
                    </article>
                </section>
                <br>
                <!--  <section class="row">
                     <article class="col-sm-12">
                     	<div class="youtube" id="sjQShSoR7e4" style="width: 100%; height: 249px;"></div>
                    </article>
                </section>
                <br>
                 <section class="row">
                     <article class="col-sm-12">
                     	<div class="youtube" id="sjQShSoR7e4" style="width: 100%; height: 249px;"></div>
                    </article>
                </section> -->
        </div>
    </div>
</div>